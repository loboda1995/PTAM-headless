...
Tracker::Tracker(ImageRef irVideoSize, const ATANCamera &c, Map &m, MapMaker &mm, VideoSource &v, const std::string &pathFile) :
        mMap(m),
        mMapMaker(mm),
        mCamera(c),
        mVideoSource(v),
...
void Tracker::TrailTracking_Start() {
    ...
    for (unsigned int i = 0;
         i < mCurrentKF.aLevels[0].vCandidates.size(); i++)  // Copy candidates into a trivially sortable vector
...
void Tracker::TrackMap() {
    ...
    for (unsigned int i = 0; i < mMap.vpPoints.size(); i++) {
        ...
        TData.nSearchLevel = TData.Finder.CalcSearchLevelAndWarpMatrix(TData.Point, mse3CamFromWorld,
                                                                       TData.m2CamDerivs);
